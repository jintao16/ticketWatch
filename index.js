...
let questions = [
    ...
    },
    {
        type: 'input',
        name: 'from_station',
        message: '输入始发站拼音-from_station(如武汉站:wuhan)：',
        validate(input) {
    ...
    },
    {
        type: 'input',
        name: 'end_station',
        message: '输入终点站拼音-end_station(如广州南站:guangzhounan)：',
        validate(input) {
            if (_stations.stationInfo[input]) {
                let temp = _stations.stationInfo[input];
                if (Object.prototype.toString.call(temp) === '[object Array]') {
    ...
    },
    {
        type: 'input',
        name: 'need_tickets',
        message: '输入想买的座位类型(如:二等座|硬座，多个车次用|分开)：',
        validate(input) {
            return true;
        }
    },
...
function getLeftTicketUrl(callback) {
    request.get("https://kyfw.12306.cn/otn/leftTicket/init", (e, r, b) => {
        const defaultUrl = 'leftTicket/query';
        if (e) {
...
function queryTickets(config) {
    ...
    var req = https.get(options, function (res) {
        ...
        res.on('end', function () {
            ...
            jsonData = analyzeTicket(trainData, trainMap);
            ...
            for (var j = 0; j < train_arr.length; j++) {
                let cur_train = jsonMap[train_arr[j]];//当前车次所有信息
                let needTicket = config.need_tickets;//当前车次
                let cur_tranin_code = cur_train.station_train_code;
                let yz;//硬座数目
                let yw;//硬卧数目
                let trainNum;//车次
                if (!cur_train) {
                ...
                yz = cur_train.yz_num;//硬座数目
                yw = cur_train.yw_num;//硬卧数目

                let trains = {
                    '当前车次': cur_tranin_code,
                    '高级软卧': cur_train.gr_num, '其他': cur_train.qt_num, '软卧': cur_train.rw_num, '软座': cur_train.rz_num,
...
fs.readFile('config.json', 'utf-8', (error, data) => {
    if (error || !data || isRewrite) {
        prompt(questions).then(answer => {
            ...
            answer.need_tickets = answer.need_tickets.split('|');
